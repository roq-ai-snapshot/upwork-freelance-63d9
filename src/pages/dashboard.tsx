...
import { getAgencyMembers } from 'apiSdk/agency-members';
import { AgencyMemberInterface } from 'interfaces/agency-member';

import { AgencyListPage as ShowCaseList } from 'pages/agencies';

function DashboardPage() {
  const { session } = useSession();

  const agencyStat = useSWR<PaginatedInterface<AgencyInterface>>(
    () => `/agencies`,
    () => getAgencies({ limit: 1, offset: 0 }),
  );
  ...
  const agencyMemberStat = useSWR<PaginatedInterface<AgencyMemberInterface>>(
    () => `/agency-members`,
    () => getAgencyMembers({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Agencies',
      result: agencyStat?.data?.totalCount,
      isLoading: agencyStat.isLoading,
    ...
    },
    {
      ...
      result: agencyMemberStat?.data?.totalCount,
      isLoading: agencyMemberStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`agencies`} passHref legacyBehavior>
                  <Button
