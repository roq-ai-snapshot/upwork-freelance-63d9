...
import { BidListPage } from 'pages/bids';
...
function ProjectViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ProjectInterface>(
    ...
    () =>
      getProjectById(id, {
        relations: ['user_project_client_idTouser', 'user_project_freelancer_idTouser'],
      }),
  );
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Project Client Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_project_client_idTouser?.id}`}>
                        {data?.user_project_client_idTouser?.email}
                      </Link>
                    }
                  />
                )}
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Project Freelancer Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_project_freelancer_idTouser?.id}`}>
                        {data?.user_project_freelancer_idTouser?.email}
                      </Link>
                    }
                  />
                )}
