...
export function ProjectListPage(props: ProjectListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getProjects({
        relations: ['user_project_client_idTouser', 'user_project_freelancer_idTouser', 'bid.count'],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_project_client_idTouser',
          header: 'User Project Client Id Touser',
          accessorKey: 'user_project_client_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_project_client_idTouser?.id}`}
            >
              {record.user_project_client_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_project_freelancer_idTouser',
          header: 'User Project Freelancer Id Touser',
          accessorKey: 'user_project_freelancer_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_project_freelancer_idTouser?.id}`}
            >
              {record.user_project_freelancer_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('bid', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Bid',
          accessorKey: 'bid',
          cell: ({ row: { original: record } }: any) => record?._count?.bid,
        }
      : null,
